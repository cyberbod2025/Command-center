...
## Estado real de conexión por frente (verificado 2026-07-31 vía API)
...
No se ejecuta el paso 4–6 (Vercel) para un frente cuyo `DECISIONES.md` tenga una decisión vigente que aplace el despliegue (caso Proyecto Horizonte, D-005). Un build verde no anula una decisión aprobada.

---

## Proyectos huérfanos (Vercel/Supabase sin frente asignado)

Ver R-10 en `RIESGOS.md`. Identificados por carpeta local — son proyectos previos e independientes, **no forman parte de los cuatro frentes del portafolio**. No se tocan, no se pausan, no se enlazan a ningún frente sin que Hugo lo pida explícitamente:

- Supabase: `FERIA` (↔ carpeta `Diagnostico-Colectivo`/`feria-alternativa`, no confirmado 1:1), `SASE` (`uvnetpnjinxzhggoqmwz`, INACTIVE — proyecto legacy, no confundir con el frente SASE Zero que usa `SASE-Light`), `Laboratorio-digital-310` (INACTIVE)
...
Conclusión: ninguno de estos proyectos huérfanos es en realidad uno de los cuatro frentes con nombre distinto. Son trabajo previo legítimo fuera del alcance actual del Command Center.
